    ...
    var body: some View {
        VStack(alignment: .leading, spacing: lineSpacing) {
            ForEach(parseMarkdownLines(), id: \.id) { line in
                DataStructureMarkdownLineView(line: line, font: font, foregroundColor: foregroundColor)
            }
        }
    }
    
    private func parseMarkdownLines() -> [DataStructureMarkdownLine] {
        let lines = markdown.components(separatedBy: .newlines)
        return lines.enumerated().map { index, content in
            DataStructureMarkdownLine(id: UUID().uuidString, content: content)
        }
    }
}

struct DataStructureMarkdownLine: Identifiable {
    let id: String
    let content: String
}

struct DataStructureMarkdownLineView: View {
    let line: DataStructureMarkdownLine
    let font: Font
    let foregroundColor: Color
