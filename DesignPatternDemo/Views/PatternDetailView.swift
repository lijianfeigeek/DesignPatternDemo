...
struct MarkdownText: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: lineSpacing) {
            ForEach(parseMarkdownBlocks(), id: \.id) { block in
                MarkdownBlockView(block: block, font: font, foregroundColor: foregroundColor)
            }
        }
    }
    ...
    private func parseMarkdownBlocks() -> [MarkdownBlock] {
        ...
        return blocks.enumerated().map { index, content in
            MarkdownBlock(id: UUID().uuidString, content: content.trimmingCharacters(in: .whitespacesAndNewlines))
    ...
    var body: some View {
        Group {
            if block.content.hasPrefix("```") {
                // 代码块
                CodeBlockView(code: extractCodeBlock(block.content))
            } else if block.content.hasPrefix("#") {
                // 标题
                HeaderView(text: extractHeaderText(block.content), level: extractHeaderLevel(block.content))
                ...
                // 项目符号
                BulletPointView(text: extractBulletText(block.content))
            } else if block.content.contains("|") && block.content.trimmingCharacters(in: .whitespaces).hasPrefix("|") {
                // 表格
                TableView(markdown: block.content)
            } else if block.content.trimmingCharacters(in: .whitespaces).isEmpty {
                // 空行
                EmptyView()
            } else {
                // 普通段落
                ParagraphView(text: block.content, font: font, foregroundColor: foregroundColor)
            }
        }
    }
